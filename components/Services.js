...
const Services = () => {
  return (
    <section id="service" className='w-full my-[2rem] md:my-[3rem]  animate-top'>
        <h2 className='my-[2rem] text-[2rem] md:text-[1.4rem] sm:text-[1rem] font-semibold text-center dark:text-white dark:bg-black '>What do you want us to wash?</h2>
       <div className='w-[80%] md:flex-col md:items-center mx-auto lg:flex-wrap  flex gap-5 p-1'>
        {ServiceData.map((service, i) =>(
             <Link key={i} href={service.url}>
             <div className='bg-white dark:bg-gray-800 border-2 py-[3rem] lg:w-[45%]  border-light-blue cursor-pointer w-[25%] sm:w-full md:w-[70%] h-[22rem] items-center rounded-md shadow-md p-2 flex flex-col gap-2'>
              
              <div className='w-[40%] h-[35%] p-1 relative'>
              ...
              <div className='w-full  h-[60%] p-2 flex flex-col gap-2'>
                  ...
                  <p className='text-[1.2rem] md:text-[1.2rem] sm:text-[.9rem] font-semibold dark:text-white md:text-center'>{service.description}</p>
              </div>
