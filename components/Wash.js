...
function Wash({service,tab,toggleTab,tabNum}) {
  ...
  return (
    <div className=' w-full flex p-4 gap-4 '   >
        <div onClick={()=>toggleTab(tab)} className={`w-[47%] h-[7rem] cursor-pointer dark:bg-gray-700 dark:text-white rounded-md border border-l-4 ${tabNum === tab ? 'bg-light-blue text-white border-l-black' :'' }  border-l-light-blue p-4 flex justify-between flex-col`}>
          ...
          <div className='flex justify-between items-center'>
            <p className={`${tabNum === tab ? 'text-black' :'' } text-main-blue font-medium`}>{service.isDryCleaning && 'DRY CLEANING'} </p>
            <p className='font-semibold place-items-end text-right'>NGN {service.cost}</p>
          </div>       
        </div>
        <div  className={`w-[47%] dark:bg-gray-700 rounded-md border absolute right-5 top-6 ${tabNum === tab ? 'translate-y-0' :'hidden -translate-y-[100%]' }  `}>
